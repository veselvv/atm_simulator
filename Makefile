...
CORE_SRCS = $(CORE_DIR)/card.c $(CORE_DIR)/database.c $(CORE_DIR)/file_io.c $(LIBS_DIR)/cJSON.c
CORE_HEADERS = $(CORE_DIR)/card.h $(CORE_DIR)/database.h $(CORE_DIR)/file_io.h $(LIBS_DIR)/cJSON.h
...
CORE_OBJS = $(CARD_OBJ) $(DATABASE_OBJ) $(FILE_IO_OBJ) $(CJSON_OBJ)
...
$(DATABASE_OBJ): $(CORE_DIR)/database.c $(CORE_DIR)/database.h
	$(CC) $(CFLAGS) $(INCLUDES) -c $< -o $@

$(FILE_IO_OBJ): $(CORE_DIR)/file_io.c $(CORE_DIR)/file_io.h
	$(CC) $(CFLAGS) $(INCLUDES) -c $< -o $@
